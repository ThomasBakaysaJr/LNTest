...
Each `[from, to]` means CC{from} opens a channel to CC{to}. Node numbers are 1-indexed (CC1, CC2, ..., CCn). The `nodes` field should match the `--cc-count` value. Edges are directed (the opener holds initial balance), but `push_msat` ensures bidirectional forwarding capability.
