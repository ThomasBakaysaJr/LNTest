...
```bash
sudo venv/bin/python3 lntest.py run takedown_random --num-msg 3 --topology-file topologies/ring_20.json
```

Lets researchers supply any arbitrary topology as a JSON file (passing `--topology-file` implies `--topology custom`). cc_manager is disabled, and the orchestrator builds the exact graph specified. This enables testing with real-world LN snapshots, random graphs, scale-free networks, or any theoretical model on real CLN nodes.
